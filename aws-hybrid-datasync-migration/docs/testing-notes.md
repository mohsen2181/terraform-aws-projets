...
export NFS_IP=$(aws ec2 describe-instances \
  --filters "Name=tag:Name,Values=onprem-nfs-server" "Name=instance-state-name,Values=running" \
  --query "Reservations[0].Instances[0].PrivateIpAddress" --output text --region $AWS_REGION)

echo "NFS Server ID:     $NFS_SERVER_ID (IP: $NFS_IP)"
...
> **💡 Note on Private Subnets:** Because instances reside in a private subnet without internet access for `dnf`, if `/media/data/images` was not created automatically during boot, run this one-time setup:

```bash
# Quick setup if /media/data/images does not exist:
sudo mkdir -p /media/data/images

for i in $(seq -w 1 20); do
  echo "This is migration test file $i" | sudo tee /media/data/images/file-$i.txt > /dev/null
done

echo "hello from nfs server" | sudo tee /media/data/test-nfs.txt
sudo chown -R nobody:nobody /media/data
sudo chmod -R 777 /media/data

echo "/media/data *(rw,sync,no_root_squash)" | sudo tee /etc/exports
...
#### If NOT mounted, mount it directly:

```bash
# Get NFS Server IP from your workstation (e.g. 10.0.2.181) and mount:
NFS_IP="10.0.2.181"   # Replace with your actual NFS Server Private IP if different

sudo mkdir -p /mnt/data
sudo mount -t nfs4 $NFS_IP:/media/data /mnt/data
echo "$NFS_IP:/media/data /mnt/data nfs4 defaults,_netdev 0 0" | sudo tee -a /etc/fstab

# Verify mount
df -hT /mnt/data
```

#### Inspect remote files and simulate production workload:
